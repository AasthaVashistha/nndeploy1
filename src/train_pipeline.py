...
def run_training(tol, epsilon, batch_size):
    epoch_counter = 0
    mean_seuared_error = 1
    loss_per_epoch = [mean_seuared_error]
    ...
    while True:
        ...
        for start_index in range(0, n_samples, batch_size):
            ...
            for i in range(X_batch.shape[0]):
                ...
                Y_train_i = Y_batch[i].reshape(-1, 1)
                L = 0.5 * (Y_train_i[0, 0] - h[config.NUM_LAYERS - 1][0, 0]) ** 2
                mean_seuared_error += L

                del_L_by_del_h[config.NUM_LAYERS - 1] = h[config.NUM_LAYERS - 1] - Y_train_i
        ...
        mean_seuared_error /= n_samples
        epoch_counter += 1
        loss_per_epoch.append(mean_seuared_error)

        print(f"Epoch # {epoch_counter}, Loss = {mean_seuared_error}")
